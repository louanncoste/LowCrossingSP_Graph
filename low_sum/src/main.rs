...
//removes all nodes that are internal in some path
fn remove_internal_nodes(nodelist: &mut Vec<Node>, all_node_info: &Vec<NodeInfo>){
    let mut i=0; 
...
fn compute_path_iteration(nodelist: &mut Vec<Node>, g: &Graph, all_node_info: &mut Vec<NodeInfo>, alpha: f32){
    ...
    let mut sampled_pairs=MyPairs::create_sample_pairs(nodelist, sample_size_per_vertex, all_node_info);
    //TODO if n1+alpha < something, put everything
    sampled_pairs.compute_costs(g);
   
    let nb_stop: usize = nodelist.len() /2;
    let mut nb_internal=0;

    while nb_internal < nb_stop {   
        let e: &Pair= sampled_pairs.get_minimal_cost_pair(&all_node_info); 
        let u=e.u;
        let v=e.v; 
        if u==u32::MAX {return;}

        //Add the pair e to our spanning path
        for (node,other_node) in [(e.u, e.v), (e.v,e.u)]{
            match all_node_info[node as usize].state {
                ...
                _ => panic!("Error. How did we even end up here??"),
            }
...
fn compute_path(g:&Graph, all_node_info: &mut Vec<NodeInfo>, nodelist: &mut Vec<Node>, alpha:f32)->f32{
    let mut timer:f32; 
    ...
    while nodelist.len()>=3{
        ...
        compute_path_iteration(nodelist, g, all_node_info, alpha);

        timer=now.elapsed().as_secs_f32();
        println!("Finished in {timer} sec.");
        timetaken+=timer;
    
        remove_internal_nodes(nodelist, all_node_info);
    }
    timetaken
}
...
fn main(){
    ...
    let timetaken=compute_path(&g, &mut all_node_info, &mut nodelist, alpha);
